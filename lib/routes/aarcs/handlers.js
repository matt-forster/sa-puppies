...
export default [
  ...
  function findPuppyList(req, res, next) {
    ...
    request('http://aarcs.ca/adoptable-dogs/adoptable-puppies/',
      (err, response, body) => {
        if (err) {
          res.send(400, err);
          res.end();
  ...
  function findNewPuppies(req, res, next) {
    ...
    // store the cuteness
    let getKnownPups = this.modules.puppyCrate.gatherPuppies();
    ...
    let savePups = getKnownPups.then((knownPups) => {
      ...
      return this.modules.puppyCrate.addNew(this.vars.puppies);
    });
